...
class PeripheralTest:
    ...
    def rx_write_callback(self, value: [], options):
        print('rx_write_callback, options: ', options, 'size', len(value), 'value', value)
        # minus ATT header and l2cap header
        self.MTU = options['mtu'] - 2 - 3
        print('MTU: ', self.MTU)
        self.rx_string += value.decode('utf-8')
        rx_strings = self.rx_string.split('\n')
        if len(rx_strings) > 1:
            for s in rx_strings[:-1]:
                self.on_message_received(s)
            self.rx_string = rx_strings[-1]

    def on_message_received(self, message):
        print("on_message_received", message)
        self.update_tx_value(self.tx_characteristic, message + '\n')
